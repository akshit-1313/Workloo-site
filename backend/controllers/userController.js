...
export const login = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!role || !email || !password) {
    return next(
      ...
      new ErrorHandler("All fields are required to authorize", 400)
    )
  ...
  sendToken(user, 200, res, "User logged in sucessful")
})
...
export const updateProfile=catchAsyncErrors(async(req,res,next)=>{
  ...
  if(req.user.role==="Job Seeker"&&(!firstNiche||!secondNiche||!thirdNiche)){
    return next( new ErrorHandler("Provide all your Job niches",400))
  }
  if(req.files){
